...
/* الحد الأقصى لعدد الفقرات التي يُقسَّم إليها النص (حدّه المستخدم: 50) */
const MAX_PARAGRAPHS = 50;

const SYSTEM_ANALYZE = `أنت خبير تعليمي متخصص في تحليل النصوص العربية وإنشاء اختبارات.
مهمتك: تحليل النص الذي يرسله المستخدم وإرجاع النتيجة بصيغة JSON فقط.
القواعد:
1. قسّم النص إلى فقرات منطقية مترابطة (من 3 إلى ${MAX_PARAGRAPHS} فقرة قدر الإمكان، وكلما كان النص أطول زاد عدد الفقرات). إذا كان النص كتلة واحدة متصلة، قسّمه حسب المواضيع أو مجموعات الجمل المتقاربة.
2. انسخ نص كل فقرة كما ورد حرفيًا مع تقليم المسافات الزائدة فقط.
...
4. اكتب سياقًا عامًا موجزًا (من جملة إلى ثلاث جمل) يلخص موضوع النص كله.
...
القواعد الصارمة:
1. السؤال يجب أن يكون قابلًا للإجابة من الفقرة نفسها فقط، لا تفترض معرفة المستخدم للفقرة مسبقا ولا تشر لوجودها.
...
/* --- تحليل النص: فقرات + كلمات مفتاحية + سياق عام --- */
async function aiAnalyzeText(text, signal) {
  const body = {
    messages: [
      { role: "system", content: SYSTEM_ANALYZE },
      { role: "user", content: `حلّل النص التالي:\n\n"""\n${text}\n"""` },
    ],
    temperature: 0.4,
    max_tokens: 8192,
  ...
  if (paragraphs.length > MAX_PARAGRAPHS) {
    /* دمج الفائض عن الحد في فقرة أخيرة واحدة حتى لا يُهدر أي جزء من النص */
    const head = paragraphs.slice(0, MAX_PARAGRAPHS - 1);
    head.push({
      text: paragraphs.slice(MAX_PARAGRAPHS - 1).map((p) => p.text).join("\n"),
      keywords: [],
    });
    head[head.length - 1].keywords = normalizeKeywords([], head[head.length - 1].text);
    paragraphs.length = 0;
...
function demoGenerateQuestion({ paragraph, keyword, pool, difficulty: lvl }) {
  ...
  return {
    question: `أكمل الفراغ بالكلمة الصحيحة كما وردت في الفقرة:\n«${masked}»`,
    options,
...
function loadSpeedSettings() {
  try {
    ...
    if (raw && typeof raw.on === "boolean") {
      ...
      speedSettings.secs = [7, 10, 15, 20].includes(raw.secs) ? raw.secs : 10;
    }
  } catch {}
}
...
function renderHistory() {
  ...
  const best = Math.max.apply(null, h.map((x) => x.pct || 0));

  /* ---- إحصاءات ملخصة ---- */
  const avg = Math.round(h.reduce((s, x) => s + (x.pct || 0), 0) / h.length);
  ...
  if (prevPct === null) trendEl.textContent = "—";
  ...
  /* ---- مخطط التقدم (آخر 12 نتيجة، الأقدم يمينًا) ---- */
  spark.innerHTML = "";
  h.slice(0, 12).reverse().forEach((item) => {
    const bar = document.createElement("i");
    bar.className = "bar" + ((item.pct || 0) === best ? " best" : "");
...
const state = {
  ...
  activeParagraphs: [],    /* الفقرات المدرجة فعليًا (بعد حد العدد) */
...
async function startQuiz() {
  ...
  try {
    ...
    if (demo) {
      await sleep(2600);
      analysis = demoAnalyze(text);
    } else {
      analysis = await aiAnalyzeText(text, state.abortCtrl.signal);
    }
    ...
    const fullCount = analysis.paragraphs.length;
    toast("info", `تم تحضير ${quizCountWord(activeCount)}${activeCount < fullCount ? ` من أصل ${fullCount} فقرة` : ""} — بالتوفيق!`);
  } catch (err) {
    stopPreparingAnimation();
...
function ensureQuestion(i) {
  ...
  const promise = (async () => {
    ...
    if (state.demo) {
      ...
      return { paragraphIndex: i, keyword, ...q, userAnswer: null, correct: null };
    }
...
function renderQuestion(q) {
  el("keyword-chip").classList.toggle("hidden", !q.keyword);
  el("q-keyword").textContent = q.keyword;
...
function applyAnalysis(analysis) {
  const lim = parseInt(el("question-limit").value, 10) || 0;
  state.analysis = analysis;
  state.activeParagraphs = lim > 0 ? analysis.paragraphs.slice(0, Math.min(lim, analysis.paragraphs.length)) : analysis.paragraphs;
  state.usedKeywords = analysis.paragraphs.map(() => []);
  state.roundPcts = [];
  state.round = 0;
}

/* ====== استئناف آخر نص محفوظ ====== */
...
function buildPrintSheet() {
  ...
  html += "</ol></section>";
  html += '<footer class="ps-foot">أُنشئ بواسطة «اختبرني» — الإصدار 1.7</footer>';
...
const cardsState = { cards: [], order: [], pos: 0 };

function buildFlashcards() {
  const cards = [];
  if (state.staticQuestions) {
    state.staticQuestions.forEach((q) => {
      cards.push({ keyword: q.keyword || "سؤال", sentence: q.explanation || q.question });
    });
    return cards;
  }
  (state.analysis ? state.analysis.paragraphs : []).forEach((p) => {
    const sentences = String(p.text).split(/(?<=[.!؟?])\s+/).map((s) => s.trim()).filter(Boolean);
    (p.keywords || []).forEach((kw) => {
      const target = sentences.find((s) => s.includes(kw)) || p.text;
      cards.push({ keyword: kw, sentence: target });
    });
  });
  return cards;
}
...
function renderCard() {
  const { cards, order, pos } = cardsState;
  const card = cards[order[pos]];
  el("flashcard").classList.remove("flipped");
  el("fc-keyword").textContent = card.keyword;
  const sentenceEl = el("fc-sentence");
  sentenceEl.innerHTML = "";
  sentenceEl.appendChild(highlightKeywordFrag(card.sentence, card.keyword));
...
function init() {
  ...
  el("speed-seconds").addEventListener("change", () => {
    speedSettings.secs = parseInt(el("speed-seconds").value, 10) || 10;
    saveSpeedSettings();
  });
