...
القواعد:
1. قسّم النص إلى فقرات منطقية مترابطة (من 3 إلى 12 فقرة قدر الإمكان). إذا كان النص كتلة واحدة متصلة، قسّمه حسب المواضيع أو مجموعات الجمل المتقاربة.
...
async function aiAnalyzeText(text, signal) {
  ...
  if (paragraphs.length > 12) {
    const head = paragraphs.slice(0, 11);
    head.push({
      text: paragraphs.slice(11).map((p) => p.text).join("\n"),
      keywords: [],
    });
...
function demoAnalyze(text) {
  ...
  if (parts.length > 12) {
    const head = parts.slice(0, 11);
    head.push(parts.slice(11).join(" "));
    parts = head;
  }
